...
**Goal:** Deliver a personalized 3–10 prompt, rubric-backed assessment that records auditable decisions and measured provider cost.
...
def test_adaptive_turn_requires_question_only_when_continuing():
    assert AdaptiveTurnOutput(decision="continue", confidence=0.6,
...
- [ ] **Step 3: Add minimal models and schemas.** Use a `Person` table, a `Role.rubric_version` plus JSON ladder, session `person_id`, selected-tier fields and expectation snapshots; add item `item_type` and JSON `turn_decision`; add `AIUsage` with only identifiers, operation, model, input/output tokens, and timestamp. Define a discriminated Pydantic union for `continue` (question required) and `evaluate` (evaluation required).
...
- [ ] **Step 1: Add failing mock tests** for a generated ordered ladder, forced continuation with fewer than three answers, deterministic completion at three detailed answers, and evaluation at ten answers.
- [ ] **Step 2: Run:** `pytest tests/test_mock_provider.py -v` — expect failures for missing methods.
- [ ] **Step 3: Implement the protocol and mock.** Make the mock return a stable three-tier ladder, target uncovered expectations in its question text, and return token counts with each operation. Its `advance_assessment` must never return evaluate before the supplied minimum and must evaluate when supplied `force_evaluate=True`.
- [ ] **Step 4: Run:** `pytest tests/test_mock_provider.py -v` — expect PASS.
- [ ] **Step 5: Commit** with message `feat: add adaptive AI provider contract`.
...
- [ ] **Step 1: Add failing fake-client tests.** Assert both providers parse `AdaptiveTurnOutput`, receive the full transcript and selected-tier snapshot, and map returned token usage without retaining prompt text.
- [ ] **Step 2: Run:** `pytest tests/test_openai_provider.py tests/test_anthropic_provider.py -v` — expect failures.
- [ ] **Step 3: Implement minimal prompts and parsing.** The prompt must state the selected tier, next-tier target, full transcript, allowed item type, and server `must_continue`/`must_evaluate` constraint. Keep `store=False` for OpenAI.
- [ ] **Step 4: Run:** `pytest tests/test_openai_provider.py tests/test_anthropic_provider.py -v` — expect PASS.
- [ ] **Step 5: Commit** with message `feat: support adaptive turns in real providers`.
...
- [ ] **Step 1: Write failing API tests** covering same-name profile reuse, role ladder response, tier-required session creation, completed result at answer three, continued result before the cap, forced result at answer ten, one evaluation after retry, immutable snapshots, and persisted usage rows.
- [ ] **Step 2: Run:** `pytest tests/test_people_api.py tests/test_sessions_api.py tests/test_employee_status.py -v` — expect failures.
- [ ] **Step 3: Implement routes transactionally.** Save the answer first; if no turn result exists for that item, request and persist exactly one adaptive decision plus usage. On retry, reuse a persisted decision/evaluation rather than calling the provider again. Regenerate a legacy role ladder before exposing tiers.
- [ ] **Step 4: Run:** `pytest tests/test_people_api.py tests/test_sessions_api.py tests/test_employee_status.py -v` — expect PASS.
- [ ] **Step 5: Commit** with message `feat: add adaptive assessment session API`.
...
- [ ] **Step 1: Write failing Playwright tests.** Cover entering a display name, selecting a generated tier, completing after the mock's third answer, revisiting that named profile's result, and keyboard selection/error announcements.
- [ ] **Step 2: Run:** `npm run test:e2e -- golden-path.spec.ts profile-history.spec.ts` — expect failures.
- [ ] **Step 3: Implement the smallest accessible flow.** Use labelled inputs and native buttons/radio controls; maintain focus after screen changes; use `role="alert"` for API errors. Do not display confidence, uncertainty, or internal decision metadata.
- [ ] **Step 4: Run:** `npm run build` then `npm run test:e2e -- golden-path.spec.ts profile-history.spec.ts` — expect PASS.
- [ ] **Step 5: Commit** with message `feat: add personalized adaptive assessment flow`.
...
- [ ] **Step 1: Add README documentation** for display-name prototype identity, 3–10 prompt behavior, provider token usage records, and the fact that no practical tests execute in this release.
- [ ] **Step 2: Run backend suite:** `pytest tests/ -v` — expected PASS.
- [ ] **Step 3: Run frontend verification:** `npm run build` and `npm run test:e2e` — expected PASS with backend/frontend/Postgres running.
- [ ] **Step 4: Inspect:** `git diff --check` and `git status --short` — expected no whitespace errors and only intended changes.
- [ ] **Step 5: Commit** with message `docs: explain adaptive assessment behavior`.
